...
public final class LineImpl implements Line {
    ...
    public LineImpl(
            ...
            final List<GeoCoordinates> lineS, 
    ...
    public GeoCoordinates getGeoCoordinateAlongLine(int distanceAlong) {
        try {
        String query = "SELECT"
            + " ST_AsText(ST_Transform(ST_LineInterpolatePoint(geom_3857, LEAST(1, ? / ST_Length(geom_3857))), 4326))"
            + " FROM hudhud_ways"
            + " WHERE way_id = ?";
        PreparedStatement ps = conn.prepareStatement(query);
        ps.setInt(1, distanceAlong);
        ps.setLong(2, id);
        ResultSet rs = ps.executeQuery();
        rs.next();
        String point = rs.getString(1);
        String[] parts = point.split(" ");
        double lon = Double.parseDouble(parts[0].substring(6));
        double lat = Double.parseDouble(parts[1].substring(0, parts[1].length() - 1));
        return GeoCoordinatesImpl.newGeoCoordinatesUnchecked(lon, lat);
    } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public int measureAlongLine(final double arg0, final double arg1) {
        String query = "SELECT ST_Distance("
            + " ST_ClosestPoint("
            + " geom_3857," 
            + " ST_Transform(ST_SetSRID(ST_MakePoint(?, ?), 4326),3857)"
            + " ), ST_StartPoint(geom_3857))::int AS distance"
            + " FROM hudhud_ways"
            + " WHERE way_id = ?";
        try {
            PreparedStatement ps = conn.prepareStatement(query);
